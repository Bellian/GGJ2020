...
export class LevelMap {
    ...
    json.objectData.forEach(data => {
      switch (data.type) {
        ...
        default:
          new Placeholder(
            this,
            vec2.fromValues(...data.pos) as vec2,
            data.meta
          );
          break;
      }
  ...
  createBounds() {
    ...
    World.add(PhysicsEngine.world, [
      ...
      Bodies.rectangle(width + 50, height / 2, 100, height + 200, {
        isStatic: true
      }),
  ...
  getAllLevelObjectsByType<T extends LevelObject>(
    type: typeof LevelObject
  ): T[] {
