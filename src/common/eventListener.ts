...
export class EventListener {
    ...
    on<T extends keyof EventCallbacks>(event: T, callback: (arg: EventCallbacks[T]) => void) {
    ...
    off<T extends keyof EventCallbacks>(event: T, callback: (arg: EventCallbacks[T]) => void) {
        if(this.listener[event] !== undefined){
