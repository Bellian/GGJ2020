  ...
  tick(delta: number) {
    ...
    getAllDevices()
      ...
      .forEach(device => {
        ...
        player.position = vec2.fromValues(
          player.pawn.hitBox!.position.x,
          player.pawn.hitBox!.position.y
        );
        ...
        .forEach(e => {
          if (!this.players.has(e)) {
            ...
            let player = new Player(
              level,
              vec2.clone(spawnpoints[index].position),
              Pawn,
              e.customStateData.isAngryDad
            );
