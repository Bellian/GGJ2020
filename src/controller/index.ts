...
class Controller{
    ...
    virtualController(){
        document.querySelectorAll('playscreen > controller')[0].addEventListener('touchstart', (ev) => {
            this.startPos = [(ev as TouchEvent).targetTouches[0].clientX, (ev as TouchEvent).targetTouches[0].clientY];
        });
        document.querySelectorAll('playscreen > controller')[0].addEventListener('touchmove', (ev) => {
        ...
        document.querySelectorAll('playscreen > controller')[0].addEventListener('touchend', (ev) => {
